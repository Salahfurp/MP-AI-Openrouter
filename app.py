...
Do NOT reveal chain-of-thought, internal reasoning, analysis steps, or a thinking process. Return only the final user-facing answer. Always end with a short 'Source' line listing the UPPG page numbers that support the answer.
"""
...
def clean_ai_answer(text):
    """Remove accidental reasoning/thinking text and keep only the final answer."""
    if not isinstance(text, str):
        return text

    cleaned = text.strip()

    # Prefer an explicit final-answer section if the model produced one.
    patterns = [
    ...
    # Remove common reasoning preambles/sections if they leak through.
    reasoning_starts = [
    ...
    if any(re.search(p, cleaned) for p in reasoning_starts):
        # If no explicit final section exists, avoid displaying leaked reasoning.
        # Ask the user-facing layer to show a safe fallback instead.
        return (
