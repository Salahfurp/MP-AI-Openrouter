...
def _service_key(value):
    """Stable bilingual lookup key used only inside app orchestration.

    Kept local so app.py never depends on a private helper from another module.
    """
...
def build_semantic_service_map(required_result, submitted_rows):
    ...
    for name in submitted_names:
        ...
        if official in required_names and confidence >= 0.70:
            ...
            out[_service_key(name)] = out[name]
        else:
...
"""
        try:
            ...
            for item in data:
                ...
                if submitted and official in allowed and confidence >= 0.62:
                    ...
                    out[_service_key(submitted)] = out[submitted]
        except Exception:
            pass
    ...
    for row in submitted_rows:
        mapped = out.get(row.service) or out.get(_service_key(row.service))
        if mapped:
...
def build_comparison_chat_event(submitted_rows, source_label="الجدول المقدم"):
    ...
    # Semantic matching must never be able to crash the compliance engine.
    # If the external model is unavailable or a label cannot be translated, the
    # deterministic bilingual matcher in compare_facilities remains the fallback.
    try:
...
# Clean assistant UI: no Demo/RAG modes and no workflow categories.
# The assistant decides the right specialist tool for every turn.
